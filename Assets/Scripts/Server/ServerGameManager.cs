...
public class ServerGameManager : NetworkBehaviour, IGameManager
{
    ...
    // TODO: this is now IsMatchReadyToStart in GamePlayManager
    ...
    //    _gamePlayManager.ReadyPlayerForGamePlay(networkId);//TODO, _playerSessionStatus[networkId].Designation);

    //    if (_gamePlayManager.IsMatchReadyToStart()) // TODO: this was CheckAndSendGameReadyToStartMsg
    //    {
    //        var status = _networkManager.SceneManager.LoadScene("GamePlay", LoadSceneMode.Single);
    //        if (status != SceneEventProgressStatus.Started)
    //        {
    ...
    //        }
    //    }
    //    //TODO: remove: CheckAndSendGameReadyToStartMsg(0);
    //}
    ...
    void Start()
    {
        ...
        _gamePlayManager = gameObject.AddComponent<GamePlayManager>();
        ...
        if (!ApplicationController.IsLocalTesting)
        {
            ...
            _multiplayManager.Init(_serverNetworkManager.StartServer);

            _serverNetworkManager.Init(_gamePlayManager, _multiplayManager, _multiplayManager.UpdatePlayerCount);
        } else
        {
            // local testing, so not using Unity Multiplay services
            _serverNetworkManager.Init(_gamePlayManager, null, null);
        }

        // NOTE: because of how this class is manually instantiated, don't put anything important here, use Init 
    }
